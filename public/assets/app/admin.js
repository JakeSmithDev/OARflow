...
(function () {
  ...
  const NAV_CAP = {
    ...
    developer: '*', team: ['team.manage', 'dispatch.manage'], settings: '*',
  };
  OF.hasCap = (cap) => {
    if (!cap) return true;
    const caps = (OF.session && OF.session.capabilities) || [];
    const required = Array.isArray(cap) ? cap : [cap];
    return caps.includes('*') || required.some((item) => caps.includes(item));
  };
  ...
  const ROUTES = [
    ...
    { path: '/admin/team', file: 'team', view: 'team' },
