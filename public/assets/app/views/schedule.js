...
const OF = window.OF;
    ...
    async function routeModal() {
      ...
      const selected = new Set(techFilter ? [+techFilter] : (TECHS||[]).map(t=>+t.id));
      const m = OF.modal(`<div class="modal-head"><h3>Plan routes</h3><button class="x" data-close>&times;</button></div>
        <div class="modal-body" style="min-height:220px">
          <div class="grid cols-2">
            <div class="field"><div class="row between"><label style="margin:0">Reps / technicians</label><span><button type="button" class="link-btn tiny" id="rt_all">All</button> <button type="button" class="link-btn tiny" id="rt_none">None</button></span></div><div id="rt_techs" class="card" style="padding:4px 10px;max-height:150px;overflow:auto"></div></div>
            <div><div class="field"><label>Date</label><select id="rt_date">${OF.dateSelectOptions(cursor)}</select></div>
          ...
          ${canDispatch?`<div class="row" style="gap:8px;margin:-2px 0 14px"><input id="rt_newtech" placeholder="New rep / technician name" style="max-width:280px"><button class="btn btn-secondary btn-sm" id="rt_addtech" type="button">Add rep</button></div>`:''}
          <div id="rt_out"><p class="muted small">Select reps and a date to preview drive order and nearby assignment suggestions.</p></div>
        </div>
      ...
      function renderPlan(d, key) {
        ...
        const methodNote=d.method==='coordinates'
          ? 'Routes use precise coordinates.'
          : d.method==='mixed'
            ? 'Some stops use coordinates; the rest are grouped by address similarity.'
            : 'Routes are grouped by ZIP, city, and address similarity; Google Maps resolves the final roads.';
        m.q('#rt_out').innerHTML = `<div class="row between wrap" style="gap:8px;margin-bottom:8px"><p class="tiny muted" style="margin:0">${OF.escape(methodNote)} Drive order follows scheduled appointment times.</p>${proposed?`<span class="badge ok no-dot">${proposed} suggested assignment${proposed===1?'':'s'}</span>`:''}</div>
          ${hasStops?(d.routes||[]).map(r=>`<div class="card" style="margin-bottom:10px;overflow:hidden"><div class="card-head" style="min-height:auto;padding:10px 12px"><span class="m-dot" style="background:${OF.color(r.technician.color)};width:10px;height:10px;border-radius:50%"></span><h4 style="margin:0">${OF.escape(r.technician.name)}</h4><span class="tiny muted" style="margin-left:auto">${r.stops.length} stop${r.stops.length===1?'':'s'}${r.totalMiles!=null?` · ${r.totalMiles} mi`:''}</span></div>
      ...
      async function run(apply=false) {
        ...
        m.q('#rt_out').innerHTML='<div class="loading-page" style="min-height:100px"><span class="spinner"></span></div>';
        try {
          const d=apply
        ...
        } catch(e){ m.q('#rt_out').innerHTML=`<p class="muted small">${OF.escape(e.message)}</p>`; }
      }
    ...
    async function render(root) {
      ...
      root.innerHTML = `
        <div class="sched-toolbar">
          ...
          <button class="btn btn-secondary btn-sm" id="routeBtn" title="Plan routes and assign nearby stops">${OF.icon('pin',14)} Routes</button>
