...
const OF = window.OF;
    ...
    // Curated catalog of common professional pest-control products. EPA reg #s
    // verified against EPA/manufacturer SDS labels (June 2026). Used to autofill
    // the Add-product form; users can still edit anything or add custom products.
    const PEST_PRODUCTS = [
    ...
    function productModal(id, p = {}) {
      const m = OF.modal(`<div class="modal-head"><h3>${id ? 'Edit' : 'Add'} product</h3><button class="x" data-close>&times;</button></div>
        <div class="modal-body">
          <div class="field"><label>Name *</label><input id="p_name" list="pestcatalog" autocomplete="off" placeholder="${id ? '' : 'Start typing — e.g. Termidor, Talstar…'}" value="${OF.escape(p.name || '')}">
            <datalist id="pestcatalog">${PEST_PRODUCTS.map((x) => `<option value="${OF.escape(x.name)}">${OF.escape(x.ai)}</option>`).join('')}</datalist>
            ${id ? '' : '<div id="p_auto" class="tiny" style="color:var(--brand);margin-top:4px;display:none">✓ Autofilled from catalog — edit anything as needed.</div>'}</div>
      ...
      if (!id) m.q('#p_name').addEventListener('input', (e) => {
        ...
        const note = m.q('#p_auto'); if (note) note.style.display = 'block';
      });
