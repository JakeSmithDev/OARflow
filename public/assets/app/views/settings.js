...
const OF = window.OF;
    ...
    function integrationsTab(root){
      ...
      root.innerHTML = card('Stripe payments', `<p class="muted small" style="margin-top:0">Accept card payments on invoices and bill recurring plans. ${ig.stripeEnabled?'<span class="badge ok no-dot">Connected</span>':'<span class="badge neutral no-dot">Not connected</span>'}</p>
        ...
        <div class="grid cols-2">${field('From number',`<input id="sm_from" value="${OF.escape(ig.smsFrom||'')}" placeholder="+15551234567">`)}${field('Messaging Service SID (optional)',`<input id="sm_mss" placeholder="MG…">`)}</div>
      ...
      root.querySelector('#saveEmail').onclick=async()=>{await OF.put('/api/admin/settings/integrations/email',{from:root.querySelector('#em_from').value});OF.toast('Saved','ok');};
    }
