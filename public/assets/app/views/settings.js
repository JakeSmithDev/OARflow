...
const OF = window.OF;
    ...
    function bookingTab(root){
      ...
      root.innerHTML = card('Booking', `
        ...
        <div class="row" style="gap:10px;margin:6px 0 14px;padding:11px 12px;border:1px solid color-mix(in srgb,var(--brand) 20%,var(--line));border-radius:10px;background:var(--brand-tint-2)">${OF.icon('pin',16)}<div><div class="cell-strong">Service address required</div><div class="tiny muted">Every new customer and booking must include a service location for scheduling and routing.</div></div></div>
        ...
        <div class="grid cols-2">${field('Suggested start times', `<select id="av_start_interval">${[15,30,45,60,90,120].map(minutes=>`<option value="${minutes}" ${Number(av.startTimeIntervalMinutes||30)===minutes?'selected':''}>Every ${minutes} minutes</option>`).join('')}</select>`,'Controls the quick choices when staff add an appointment. Any minute can still be entered manually.')}${field('Slot length (minutes)', `<input id="av_slot" type="number" min="15" step="15" value="${av.slotMinutes}">`,'Used for precise time slots when a service has no duration.')}</div>
        ${field('Crews / capacity', `<input id="av_cap" type="number" min="1" value="${av.capacityPerSlot}">`,'How many jobs can run in the same slot/window.')}
      ...
      root.querySelector('#saveBooking').onclick=async()=>{await OF.put('/api/admin/settings/settings',{booking:{defaultMode:root.querySelector('#bk_mode').value,requestSlotCount:+root.querySelector('#bk_count').value,leadTimeHours:+root.querySelector('#bk_lead').value,maxDaysOut:+root.querySelector('#bk_max').value,minimumCancelNoticeHours:+root.querySelector('#bk_cancel').value||0,collectAddress:true,confirmationMessage:root.querySelector('#bk_msg').value}});OF.toast('Saved','ok');};
      ...
      root.querySelector('#saveAvail').onclick=async()=>{const hours={};DAYS.forEach((_,i)=>{const closed=root.querySelector(`.h_closed[data-d="${i}"]`).checked;hours[i]=closed?[]:[{start:root.querySelector(`.h_start[data-d="${i}"]`).value,end:root.querySelector(`.h_end[data-d="${i}"]`).value}];});const windows=wins.filter(w=>w.label&&w.start&&w.end);await OF.put('/api/admin/settings/settings',{availability:{slotMinutes:+root.querySelector('#av_slot').value,startTimeIntervalMinutes:+root.querySelector('#av_start_interval').value,capacityPerSlot:+root.querySelector('#av_cap').value,granularity:root.querySelector('#av_gran').value,windows,hours}});OF.toast('Saved','ok');};
